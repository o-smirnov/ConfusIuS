...
double initialization(void * ParamsArray){

	struct PD_params *p= (struct PD_params *) ParamsArray;

	int N=p->inbetween_size;
	
	p->f =(double *) malloc(sizeof(double)*N);
	p->w =(double *) malloc(sizeof(double)*N);
	
	double Delta=(p->d_max)/(N-1);
	if(input_fw(FFTW_FORWARD,N,0.0,Delta,p->f)==-1.0) exit(0);
    	if(input_fw(FFTW_BACKWARD,N,0.0,Delta,p->w)==-1.0) exit(0);
...
double ComputePD_LH(int Nbins, double * DataArray,double *result,  void * ParamsArray  ) {
	    ...
	    for (int i = 0; i < N; i++)
	    		{
	    	       ...
	    	       if((p->f[i])< (p->d_min) ){
	    		       ...
	    		       in[i]=Get_Rx(p->f[i],ParamsArray)*dx*amp_I;
	    		   }
    ...
    double temp_real;
		...
		inbetween[i][0]=cos(inbetween[i][1]/amp_I)*exp(temp_real/amp_I-nbar/amp_I-0.5*pow(noise*p->w[i]*2*PI,2))*amp_II;
		inbetween[i][1]=sin(inbetween[i][1]/amp_I)*exp(temp_real/amp_I-nbar/amp_I-0.5*pow(noise*p->w[i]*2*PI,2))*amp_II;
				  }
    ...
    for (int i = 0; i < N; i++)
    		{
    		  ...
    		  if((p->f[i]>p->stk_model.Smin)&&(p->f[i]< p->stk_model.Smax)){
    			  inStacking[i]=dx*Get_dNdS_per_px(p->f[i],p->pixelsize,&(p->stk_model))*pow(3600.0/p->pixelsize,2);
    		  }else{
    			  inStacking[i]=0.0;
    		  }
/*
...
//	printf("nbar=%lf nbar_stk=%lf\n",nbar, nbar_stk);
	...
	double normalization=exp(nbar)-1.0;
	...
	double noise=p->sigma_noise; //unit Jy
		  ...
		  for (int i = 0; i < N; i++){
			...
			double  R_pd=cos(p->inbetween[i][1])*exp(p->inbetween[i][0])-1.0;
			double  I_pd=sin(p->inbetween[i][1])*exp(p->inbetween[i][0]);

			double R_stk=inbetweenStacking[i][0]/nbar_stk*exp(-0.5*pow(noise*p->w[i]*2*PI,2));
			double I_stk=inbetweenStacking[i][1]/nbar_stk*exp(-0.5*pow(noise*p->w[i]*2*PI,2));

			double R_norm= nbar/(pow(p->inbetween[i][0],2)+pow(p->inbetween[i][1],2))*p->inbetween[i][0];
			double I_norm=-nbar/(pow(p->inbetween[i][0],2)+pow(p->inbetween[i][1],2))*p->inbetween[i][1];
		//	double R_norm=1.0;
		//	double I_norm=0.0;

			double R_ext=R_norm*R_stk-I_norm*I_stk;
			double I_ext=R_norm*I_stk+R_stk*I_norm;

			//Stacking
			inbetween[i][0]=(R_pd*R_ext-I_pd*I_ext)/normalization;
			inbetween[i][1]=(R_pd*I_ext+R_ext*I_pd)/normalization;
...
double Joined_LH(int Nbins_pd, double * DataArray_pd,  int Nbins_stk, double * DataArray_stk,void * ParamsArray){

 
	double LogLH,LogLH_pd,LogLH_stk;
	double result_pd[Nbins_pd];
	double result_stk[Nbins_stk];

	


	LogLH_pd=ComputePD_LH(Nbins_pd, DataArray_pd,result_pd, ParamsArray );

	LogLH_stk=ComputeStk_LH(Nbins_stk, DataArray_stk,result_stk, ParamsArray );
		

	//Save curves
	double x[3][Nbins_stk];
     	memcpy(x, DataArray_stk, sizeof(double)*3*Nbins_stk);
	

    	FILE *fpw;
    	fpw=fopen("./result_stk.txt","w");
    	for(int i=0;i<Nbins_stk;i++){
    		fprintf(fpw,"%e   %e   %e \n",(x[0][i]+x[1][i])*0.5,x[2][i],result_stk[i]);
    	}
    	fclose(fpw);
	...
	printf("PofD Loglikelyhood is %e\n",LogLH_pd );
	printf("Stacking Loglikelyhood is %e\n",LogLH_stk );
	LogLH=LogLH_pd+LogLH_stk;
